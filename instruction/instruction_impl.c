...
{
    *fetch_reg_16(a, i->args[0].value.reg) = *((uint16_t*)&a->mem->mem[a->SP]);
    a->SP += 2;
};
...
void execute_PUSH(arcade_t *a, struct ins *i) 
{
   ...
   *((uint16_t*)&a->mem->mem[a->SP-2]) = *fetch_reg_16(a, i->args[0].value.reg);
   a->SP -= 2;
};
...
{
    *((uint16_t*)&a->mem->mem[a->SP-2]) = a->PC + i->bytecode_size;
    a->SP -= 2;
    a->PC = i->args[0].value.imm16 - i->bytecode_size;
};
